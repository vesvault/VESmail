...
#define	snif_cert_APIERR(cert, curlerr, rcode)	((cert)->tlserr.line = __LINE__, (cert)->tlserr.code = (curlerr == CURLE_OK ? rcode : curlerr))
#define	snif_cert_IOERR(cert)			((cert)->tlserr.line = __LINE__, (cert)->tlserr.code = errno, SNIF_CE_IO)
...
const char *snif_cert_alloccn(snif_cert *cert) {
    ...
    snif_cert_APIERR(cert, curlerr, code);
...
int snif_cert_savecrt(snif_cert *cert) {
    ...
    if (!bio) return cert->error = snif_cert_IOERR(cert);
    ...
    BIO_free(bio);
    if (w < 0) return cert->error = snif_cert_IOERR(cert);
    free(cert->unsaved);
    cert->unsaved = NULL;
    return w;
}
...
int snif_cert_download(snif_cert *cert) {
	...
	rs = 0;
	snif_cert_APIERR(cert, curlerr, 0);
	cert->error = SNIF_CE_API;
    }
    free(cbuf);
...
void *snif_cert_pkey(snif_cert *cert) {
	...
	if (cn && RSA_generate_key_ex(rsa, SNIF_CERT_RSA_BITS, e, NULL) > 0) {
		...
		if (snif_cert_sendreq(cert, req)) {
		    ...
		    if (pkey) cert->error = snif_cert_IOERR(cert);
		}
	    ...
	    RSA_free(rsa);
	    if (cn) cert->error = (snif_cert_TLSERR(cert), SNIF_CE_LIB);
	}
	BN_free(e);
    }
    return cert->pkey;
